...
The **non-metric multi-dimensional scaling (NMDS)** is, as the name implicates, the MDS for non-metric data (recall, the widely used MDS is the PCoA).

According to the definition from [Wikipedia](https://en.wikipedia.org/wiki/Multidimensional_scaling#:~:text=of%20the%20dissimilarities.-,Non%2Dmetric%20multidimensional%20scaling%20(NMDS)), the NMDS finds a non-parametric monotonic relationship between the dissimilarities in the item-item matrix and the Euclidean distance between items, and the location of each item in the low-dimensional space.
...
- it is a grafical representation, it has less precision and accountability for further usage of the analysis result in compare to the PCA.
